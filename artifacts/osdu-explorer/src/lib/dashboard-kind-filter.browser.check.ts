...
async function evaluateWithTimeout<T>(
  ...
  return Promise.race([
    evaluate<T>(client, expression),
    delay(timeoutMs).then(() => {
      throw new Error(`Browser evaluation exceeded ${timeoutMs}ms and may be unresponsive`);
    }),
  ]);
}
...
function timedBrowserAction(fn: (...args: any[]) => unknown, ...args: unknown[]): string {
  const action = browserFunction(fn, ...args);
  return `(() => {
    ...
    const heartbeat = window.setInterval(() => { heartbeatCount += 1; }, 16);
    ...
    await waitFor(
      () => evaluate<boolean>(browser!, browserFunction(() => {
        ...
        return status?.textContent?.includes("Loading Kind data: page 2 of 2")
          ...
          && document.querySelector('[data-activity-progress="true"]') !== null;
      })),
      "the initial Kind scan to show the pending later page and activity progress",
    );
    await evaluate<void>(browser, browserFunction(() => {
